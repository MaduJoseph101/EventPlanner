  ...
  return (
    <div className="min-h-screen bg-[#222831] text-white p-4 sm:p-8 flex flex-col items-center justify-center md:p-10 lg:p-12">
      <div className="bg-[#393E46] p-6 sm:p-8 rounded-xl shadow-lg w-full max-w-md md:max-w-2xl text-center">
        ...
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-2 text-[#FFD369] md:text-5xl">Registration Successful!</h1>
        <p className="text-gray-300 mb-6 sm:mb-8 md:text-lg">You're all set. Here are your event details.</p>

        {/* Ticket Stub */}
        <div className="rounded-lg shadow-inner p-6 sm:p-8 text-left">
          {details.imageUrl && ( 
            <img
              className='w-full h-48 md:h-64 object-cover object-center rounded-lg mb-6'
          ...
          <div className=" flex flex-col gap-4 md:gap-6">
            <p className="text-sm text-gray-400 uppercase tracking-wider md:text-md">Event</p>
            <h2 className="text-2xl font-bold text-white mb-4 md:text-3xl">{details.title}</h2> 
            ...
            <div className="flex items-center text-gray-300">
              ...
              <span className='md:text-lg'>{details.location}</span>
            </div>
            
            <div className="flex items-center text-gray-300 mb-3">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-[#e4a238] md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
              <span className='md:text-lg'>{details.date}</span>
            </div>
          ...
          <div>
            <p className="text-sm text-gray-400 uppercase tracking-wider mb-4 md:text-md">Registered To</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-3 gap-x-6">
              <div>
                <p className="text-xs text-gray-400 md:text-sm">Full Name</p>
                <p className="font-semibold capitalize md:text-lg">{details.fullname}</p>
              </div>
              <div>
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          <p className='md:text-md'>You will be redirected to the events page in <span className="font-bold text-[#e4a238]">{countdown}</span> seconds...</p>
        </div>
      </div>
    </div>
  );
